...
import { BarChart3, Users, Target, Activity as ActivityIcon, Settings, Bell, Search, LogOut } from "lucide-react";
    ...
    >
      <SidebarProvider>
        <div className="min-h-screen flex w-full bg-slate-50">
                        ...
                        <SidebarMenuItem key={item.title}>
                          <SidebarMenuButton
                            ...
                            className={`hover:bg-blue-50 hover:text-blue-700 transition-all duration-200 rounded-xl mb-1 ${
                              location.pathname.startsWith(item.url) ? "bg-blue-50 text-blue-700 shadow-sm" : ""
                            }`}
                          >
                ...
                {user && ( // Only show user info if logged in
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-10 h-10 bg-gradient-to-r from-slate-200 to-slate-300 rounded-full flex items-center justify-center">
                      <span className="text-slate-700 font-semibold text-sm">{user.name ? user.name[0].toUpperCase() : 'U'}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-slate-900 text-sm truncate">{user.name || 'User'}</p>
                      <p className="text-xs text-slate-500 truncate">{user.email}</p>
                    </div>
                    <Button variant="ghost" size="icon" onClick={onLogout} className="hover:bg-red-50 text-red-500 hover:text-red-700">
          ...
          {/* END OF SIDEBAR COMPONENT HERE */}

          {/* MAIN CONTENT STARTS HERE (as a sibling to Sidebar) */}
